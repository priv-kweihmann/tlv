...
class TLVFinding(object):
    ...
    def __init__(self, first, second, match, _id):
        ...
        self.file_this_start_line, self.file_this_start_col = self.__get_pos(first, match.a)
        self.file_other_start_line, self.file_other_start_col = self.__get_pos(second, match.b)
        self.file_this_stop_line, self.file_this_stop_col = self.__get_pos(first, match.a + match.size)
        self.file_other_stop_line, self.file_other_stop_col = self.__get_pos(second, match.b + match.size)
        self.text_this = TLVFinding.get_text(first, match.a, match.a + match.size)
        self.text_other = TLVFinding.get_text(second, match.b, match.b + match.size)
        self.id = _id
    ...
    def PrintDetails(self, args):
        if args.details:
            print(">>> {}".format(self.text_this.replace(os.linesep, os.linesep + ">>> ")))
            print("<<<")
            if self.id == TLVFinding.ID_TLV:
                print(">>> {}".format(self.text_other.replace(os.linesep, os.linesep + ">>> ")))
                print("<<<")
